...
public class InvokeVirtualCmd extends TwoOperandCmd {
	...
	public void execute(StackFrame frame, ExecutionResult result) {
		//先得到对该方法的描述
		MethodRefInfo methodRefInfo = (MethodRefInfo)clzFile.getConstantPool().getConstantInfo(this.getIndex());
		...
		while(currentClassName != null){
			...
			m = currentClassFile.getMethod(methodName, methodRefInfo.getParamAndReturnType());
			if(m != null){
				break;
			} else{
				//查找父类
